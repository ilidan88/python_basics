...
RE_LOG = re.compile(r"(^\S+)[\s-]*\[(\S+\s+\+\d+)\]\s*\"(\w+)\s*(\S+)\s*\S*\s*(\d+)\s*(\d+)")

url = 'https://github.com/elastic/examples/raw/master/Common%20Data%20Formats/nginx_logs/nginx_logs'

content = requests.get(url).text

for arg in RE_LOG.findall(content):
    remote_addr, request_datetime, request_type, requested_resource, response_code, response_size = arg
